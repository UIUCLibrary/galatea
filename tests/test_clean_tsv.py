...
def test_clean_tsv(monkeypatch):
    ...
    monkeypatch.setattr(clean_tsv, "get_field_names", Mock(return_value=[]))
    with patch("galatea.clean_tsv.open", mock_open(read_data="")):
        clean_tsv.clean_tsv(
...
def test_get_field_names_calls_fp_strategy():
    strategy = Mock()
    with patch("galatea.clean_tsv.open", mock_open()):
        clean_tsv.get_field_names("somepath", "excel-tab", strategy)
    strategy.assert_called_once()
...
def test_get_field_names_fp():
    with io.StringIO() as buff:
        ...
        buff.seek(0)
        assert clean_tsv.get_field_names_fp(buff, dialect="excel-tab") == [
            "1",
            "50",
            "100",
            "110",
...
def test_get_field_names_fp_raise_if_no_field_names_found():
    with pytest.raises(ValueError):
        with io.StringIO() as buff:
            clean_tsv.get_field_names_fp(buff, dialect="excel-tab")
...
def test_clean_tsv_row_diff_report_generator_called(monkeypatch, marc_entry):
    ...
    monkeypatch.setattr(clean_tsv, "write_tsv_file", write_tsv_file)
    monkeypatch.setattr(
        clean_tsv, "get_field_names", Mock(return_value=marc_entry.keys())
    )
    row_one = clean_tsv.TableRow(1, marc_entry)
