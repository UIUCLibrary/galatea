...
class TestCachedApiCheck:
    ...
    def test_reuse_cached_value(self):
        ...
        requesting_strategy = Mock(
            name="requesting_strategy", return_value=response
        )
        cached_api = validate_authorized_terms.CachedApiCheck(
...
def test_validate_authorized_terms(monkeypatch):
    monkeypatch.setattr(
        ...
        "__iter__",
        Mock(return_value=iter([(1, "abc", "efh")])),
    )
    check_terms = Mock(name="check_terms")

    monkeypatch.setattr(validate_authorized_terms, "check_terms", check_terms)
    validate_authorized_terms.validate_authorized_terms(pathlib.Path("spam"))
    check_terms.assert_called_once()


def test_validate_authorized_terms_logs_if_return_false(monkeypatch, caplog):
    monkeypatch.setattr(
        validate_authorized_terms.IterTerms,
        "__iter__",
        Mock(return_value=iter([(1, "abc", "efh")])),
...
class TestIterTerms:
    def test_iter(self):
        ...
        term_interator.iter_rows = Mock(
            return_value=iter([
                Mock(
                    line_number=1,
                    entry={"264$a": "one", "264$b": "two", "264$c": None},
                )
            ])
        )
    ...
    def test_iter_rows(self):
        ...
        validate_authorized_terms.IterTerms.tsv_file_row_iterator = Mock(
            return_value=[{"264$a": "one", "264$b": "two"}]
        )
        with patch(
            "galatea.validate_authorized_terms.open", mock_open(read_data="")
        ):
