...
def test_add_space_after_comma():
    ...
    assert (
        modifiers.add_comma_after_space(starting)
...
def test_regex_transform():
    ...
    assert (
        modifiers.regex_transform(
            starting, pattern=r"(--)(?=[A-Z])", replacement=" "
        )
        == "spam Upper"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Illinois State Geological Survey,publisher.",
            "Illinois State Geological Survey,",
        ),
        (
            "Carey, Mathew, 1760-1839.General atlas||Bower, John",
            "Carey, Mathew, 1760-1839.General atlas||Bower, John",
        ),
    ],
)
