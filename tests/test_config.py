...
class TestHomeDirectoryConfigStrategy:
    ...
    def test_read(self):
        config_format_strategy = Mock(
            spec=config.ConfigFileFormatStrategy,
            get_file_name=Mock(return_value="test_config.toml"),
        )

        strategy = config.HomeDirectoryConfigStrategy(
            config_format_strategy=config_format_strategy
        )
    ...
    def test_write_calls_serialized(self):
        ...
        config_format_strategy.serialize.assert_called_once_with({
            "spam": "eggs"
        })
...
class TestJSONConfigStrategy:
    def test_serialize(self):
        ...
        assert (
            config_format.serialize(current_config)
            == """
{
    "get_marc_server_url": "some_url"
}
